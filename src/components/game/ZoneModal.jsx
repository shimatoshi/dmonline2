...
export const ZoneModal = ({ title, cards, zoneName, selectedCard, onClose, onCardTap, onToggleFace, isOpponent = false }) => {
  return (
      ...
      <div style={{ 
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px", alignContent: "start", paddingRight: "5px" }}>
          {cards.map((item, i) => {
            ...
            // 相手の非公開カードはカード裏面を表示する
            const showBack = isOpponent && isFaceDown;
            const imgSrc = showBack ? CARD_BACK_URL : getProxyImageUrl(url);
            ...
            return (
              <div key={i} onClick={(e) => onCardTap(e, zoneName, i, item)} style={{ position: "relative" }}>
                 <img src={imgSrc} style={{
                   ...
                   // 自分の非公開カードは少し暗くして区別する
                   filter: (!isOpponent && isFaceDown) ? "grayscale(50%) brightness(0.7)" : "none"
                 }} />

                 {/* 自分の非公開（裏向き）カードにはマークを重ねて表示 */}
